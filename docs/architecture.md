...
4. **Chamfer signature shape is fixed-size elements only.** The two-layer chamfer-border technique (see Chamfer System below) is fragile combined with CSS Grid's `align-items: stretch` on variable/content-driven height — caused three real rendering bugs across the Work and Projects redesigns. Standing rule: chamfer for fixed-size tiles (Hero frame, `BrandMark`, `HowIBuild` bento tiles); plain rounded corners + border for anything whose height is content-driven (Work section cards, Projects editorial rows).
5. **Work section cards deliberately break the chamfer signature.** `WorkCard` uses a fake browser-chrome bar (dots + label) above each screenshot, and plain rectangular corners on purpose — a diagonal-cut corner reads as a design contradiction on something meant to look like a literal browser window.
...
6. **How I Build** (`HowIBuild.tsx`, id `approach`, index `05`, eyebrow "Philosophy", title "How I Build") — **not in the nav** (`navLinks` only has Work/Experience/Stack/Contact; this section is a deliberate closing beat right before the CTA, not a primary nav destination). See Bento/Chamfer Tiles below.
...
- `WorkCard`: `chamfer-panel` class for the hover-glow mechanic only (plain rounded corners + border, **not** the chamfer clip-path — see chamfer rule above), `BrowserChrome` fake dots+label bar, fixed `16/10`-aspect screenshot (`next/image`, `object-cover`), fixed-height (`h-36`) text block (category, name + `ArrowUpRight` hover reveal, 2-line-clamped description, first 2 tech chips pinned to the bottom via `mt-auto`). Fixed aspect + fixed text height is what keeps every row even regardless of description length — real screenshots can drop straight in later without layout math.
...
## Bento / Chamfer Tiles (`HowIBuild.tsx`)
This is the section where the real two-layer chamfer technique is actually used (see Chamfer System below) — every tile here has a fixed height (`h-80` tall / `h-48` short via `TileShell`), which is what makes the technique safe here and unsafe in Work/Projects.

- 5 `PointCard`s (data: `approachPoints` in `approach.ts`) — icon + a small custom SVG node/line diagram unique per point (`LayersGraphic`, `ConvergeGraphic`, `SchemaGraphic`, `DualRoleGraphic`, `PipelineGraphic` — same node/edge visual vocabulary as `HeroSchematic`/Work section's `MiniSchematic`, different topology per concept) + heading + description. Mixed span/height (2-col tall, 1-col tall, 1-col short) for grid rhythm.
...
## Chamfer System (`globals.css`)
A plain CSS `border` can't follow a `clip-path` diagonal chamfer cut — the cut edge just gets clipped away with no border segment drawn along it. A `::before` at `z-index: -1` doesn't fix this either (negative z-index can never paint behind its own parent's background, only above it below normal content). The reliable fix, used everywhere the chamfer appears: **two real DOM elements in plain paint order**, no z-index — a full-size `.chamfer-border`/`.chamfer-border-accent` layer painted first, then a `.chamfer-panel` content layer inset by 2px (not 1px — a 1px ring anti-aliases away on many real displays, especially along the diagonal) painted second. Hover/focus glow is a `.chamfer-panel::after` opacity fade (`.group:hover`/`.group:focus-within`), not an animated box-shadow blur radius (that repaints every frame under `backdrop-filter`, causing jank) and not a lift/translate (tried and rejected earlier — "too harsh"; hover feedback here is border/glow-only).

**Where this is safe vs. not**: fixed-size elements only (`HowIBuild` tiles via `TileShell`, `BrandMark`, the Hero portrait frame). CSS Grid's `align-items: stretch` grows a grid item to match its row's tallest sibling, but a *child* positioned via plain `margin`/inset (not `flex-grow`) inside that item does not automatically follow the stretch — this exposed the border layer as a solid wash in three separate bugs this session (Work cards, Projects rows) before the fixed/variable-height rule was established. Variable-height cards (Work section, Projects rows) use plain `rounded-xl border` + the same `.chamfer-panel`/`TileGlow` hover-glow classes, just without the clip-path — same hover feel, none of the stretch fragility.
