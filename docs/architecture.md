...
- `three` / `@react-three/fiber` — the hero's ambient particle field (`HeroCanvas.tsx`) and the 3D brand-mark centerpiece (`HeroMark3D.tsx`) that replaced the portrait photo, both lazy-mounted via their own `*Loader.tsx` (`next/dynamic`, `ssr: false`)
...
1. **Hero** (`Hero.tsx`, id `top`) — name, dual role badges ("Full Stack Developer" in accent blue / "Designer" in accent-secondary maroon — the only place role is styled per-word, not a pill), pulsing `--status-live` status line, CTA row (`View My Work` primary button wrapped in `Magnetic.tsx`, `Get in Touch` text link), stats row (`heroStats`: years experience, sites shipped, via `CountUp`) plus a live GitHub public-repo stat when `githubStats` is non-null. Background: two `aurora-blob` blurred gradients plus `HeroCanvas` particle field (three.js, client-only, lazy-loaded) — no separate SVG diagram layer any more (`HeroSchematic.tsx`, a scattered node/edge network that didn't represent anything real, was deleted rather than fixed; the particle field alone already carries the ambient-dots job). Right column: `HeroMark3D` (`src/components/three/`, lazy-loaded via `HeroMark3DLoader.tsx` the same way `HeroCanvas` is) — the brand mark (`BrandMark.tsx`'s round "R" badge, also used in the navbar/footer/favicon) scaled up into a real 3D object: a wireframe icosahedron with a soft inner core, continuous slow rotation plus a pointer-driven tilt (native `useFrame`/`state.pointer` lerp, not the DOM-level `TiltCard` — a WebGL scene needs the tilt driven inside the scene itself, not a CSS transform on the canvas element). Colors are the real accent/accent-secondary hex pair for whichever theme is active (`data-theme`, read via the same `useSyncExternalStore`/`MutationObserver` pattern as `ThemeToggle.tsx` — WebGL materials take literal hex, not CSS custom properties, so both theme's values from AGENTS.md's token table are hardcoded and switched, not read live). Falls back to a flat round badge (same look as `BrandMark`, just bigger) when WebGL is unavailable; freezes to a static (non-rotating) pose under `prefers-reduced-motion` rather than disappearing. Replaces the old portrait photo (`FramedImage` inside `TiltCard`, both deleted) — no personal photo in Hero any more. Orchestrated staggered load-in via Framer Motion `container`/`item` variants, <600ms, no bounce.
